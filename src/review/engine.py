"""Daily review engine — finds related memo groups via multiple strategies.

Outputs groups for LLM to write 1-2 sentence reviews locally.
No external AI API calls — the caller (Claude/subagent) writes the review.
"""
...
def find_groups(conn: sqlite3.Connection, count_per_strategy: int = 50
    ...
    """Find related memo groups across 4 strategies.
    ...
    # Strategy weight: same-book 40%, tag 30%, co-tag 20%, near-time 10%
    # But we need 50 per strategy, so just run each strategy independently.
    strategies = [
    ...
    for strategy_fn, target in strategies:
        collected = 0
        for _ in range(target * 3):  # 3x oversample
            if collected >= target:
                break
